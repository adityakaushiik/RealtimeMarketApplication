...
@router.post("/types", response_model=SuggestionTypeInDb, status_code=status.HTTP_201_CREATED)
...
async def get_suggestion_type(
    ...
    suggestion_type = await suggestion_service.get_suggestion_type_by_id(session, type_id)
...
async def update_suggestion_type(
    ...
    if not is_admin(user_claims):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized")
    updated_type = await suggestion_service.update_suggestion_type(session, type_id, type_data)
    if not updated_type:
        raise HTTPException(status_code=404, detail="Suggestion type not found")
    return updated_type
...
async def update_suggestion(
    ...
    updated_suggestion = await suggestion_service.update_suggestion(session, suggestion_id, suggestion_data)
