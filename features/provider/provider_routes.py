...
from features.provider.provider_schema import ProviderCreate, ProviderUpdate, ProviderInDb
...
@provider_router.post("/", response_model=ProviderInDb, status_code=status.HTTP_201_CREATED)
...
async def update_provider(
    ...
    """Update a provider"""
    provider = await provider_service.update_provider(session, provider_id, provider_data)
    if not provider:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Provider not found"
