...
async def test_dhan():
    ...
    test_instrument = Instrument(
        id=1, symbol="1333", name="HDFCBANK", exchange_id=1, instrument_type_id=1
    )
    ...
    try:
        intraday_data = await provider.get_intraday_prices(
            [test_instrument], start_date=start_time, end_date=end_time
        )
        ...
        if intraday_data and test_instrument.symbol in intraday_data:
            ...
            if candles:
                ...
                last = candles[-1]
                print(
                    f"First Candle: {first.datetime} (iso: {first.datetime.isoformat()})"
                )
                print(
    ...
    try:
        daily_data = await provider.get_daily_prices(
            [test_instrument], start_date=start_daily, end_date=now_utc
        )
