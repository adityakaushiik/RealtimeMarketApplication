...
from services.provider.provider_manager import get_provider_manager, ProviderManager
...
async def get_instrument_and_data_from_db(
    ...
    async for session in get_db_session():
        ...
        logger.info(f"Querying DB for UTC range (derived from IST date): {start_of_day_utc} to {end_of_day_utc}")
        ...
        for r in records:
            ...
            ts_utc = r.datetime.replace(tzinfo=timezone.utc) if r.datetime.tzinfo is None else r.datetime
            ...
            ts_shifted_epoch = int(dt_naive_ist.replace(tzinfo=timezone.utc).timestamp())
...
async def get_data_via_manager(instrument: Instrument, target_date_naive: datetime) -> pd.DataFrame:
    """
    ...
    logger.info(f"Fetching data via ProviderManager for {instrument.symbol} on {target_date_naive}")
    ...
    try:
        ...
        for p in price_history_list:
            ...
            p_dt_utc = p.datetime.replace(tzinfo=timezone.utc) if p.datetime.tzinfo is None else p.datetime
...
async def main():
    ...
    # 1. Get DB Data
    instrument, df_db = await get_instrument_and_data_from_db(SYMBOL_TO_CHECK, target_date)
...
#        return
    ...
    if df_dhan.empty:
        df_dhan_join = pd.DataFrame(columns=['timestamp_dhan_raw', 'open_dhan', 'close_dhan', 'volume_dhan'])
    ...
    if matches == 0 and not df_dhan.empty:
        logger.warning("No matches found. This might indicate Timezone shift issue persisted despite Provider parsing.")
    ...
    merged['time_str'] = timestamps.apply(lambda x: datetime.fromtimestamp(x, tz=timezone.utc).astimezone(pytz.timezone('Asia/Kolkata')).strftime('%H:%M:%S'))

    columns = ['time_str', 'open_db', 'open_dhan', 'close_db', 'close_dhan', 'pct_diff_close', 'volume_db', 'volume_dhan', 'diff_vol']
