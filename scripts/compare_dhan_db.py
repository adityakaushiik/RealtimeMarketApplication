...
async def get_dhan_data(instrument: Instrument, target_date_naive: datetime) -> pd.DataFrame:
    """
    ...
    if "HDFCBANK" not in instrument.symbol:
        logger.warning(f"Warning: Script assumes HDFCBANK (ID 1333). Selected instrument is {instrument.symbol}.")
...
async def main():
    ...
    if matches == 0 and not df_db.empty and not df_dhan.empty:
        logger.info("No matches with Standard Epoch. Trying Shifted Epoch (DB converted to NSE-shifted)...")
